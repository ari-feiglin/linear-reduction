...
So let us define $\Sigma$ to consist of tokens of the form $\mbt{x}$ for numbers $\ttt x$, $\mb\circ$ for operator symbols $\circ$ (addition,
multiplication, etc.), and $\mbt{x,$\circ$}$.
Then we define the initial $\beta$-reducer by
\begin{enumerate}
    \item $\hb(\mbt x,\mb\circ)=\mbt{x,$\circ$}$ for operator symbols $\circ$,
...
We also create an interpreter for LLang using menhir\cite{menhir}.
...
In this section we compare the runtimes of both implementations (one using linear reduction and one using menhir\cite{menhir}).
