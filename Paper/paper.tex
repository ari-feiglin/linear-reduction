...
\begin{enumerate}
    \item a new type,
    \item a function to determine the priority of the new type given the priorities of the original two types,
    \item and a function to determine the value of the new type given the values of the original two types.
\end{enumerate}
...
\yoni{This is a bit strange. It suggests that $\values\times \values \subseteq \values$, which is impossible. We need to properly define the relevant set here, and the domain and range of this function.}
            \ari{In general this is not an issue, you could for example start with a set $S_0=S$, and define $S_{n+1}=S_n\cup(S_n\times S_n)$, then $\values=\bigcup_nS_n$ has this property.
            I'm not saying this just to be technical, this is sort of how I imagined $\values$ being defined.
            In any case, I provided a simpler definition for $\values$ above.}
