...
Suppose $\xi=\sigma^1_{p_1}\cdots\sigma^n_{p_n}$, where $\sigma_n$ denotes the pair $(\sigma,n)$, a token and a priority.
Then $\beta(\xi)$ is defined recursively as follows:
\begin{enumerate}
...
\begin{example} \label{example:partial}
    ...
    This example would not be possible using our previous section's approach to $\beta$-reduction, or at least it would be much less efficient.
    This is because we'd have to define every possible reduction, in particular we'd have to be able to reduce $\mbt($ and $\mbt1$ (since they're both given an initial infinite priority).
    We could of course do this, and our above example would work.
    But what of examples like $(\cdots(1+1)\cdots)$?
    We'd need to reduce an arbitrary number of parentheses.
...
Our alphabet of tokens $\Sigma$, instead of having a token for every number, will just have tokens for \kinds{} (number, operator, etc.).
We also have another set $\values$, the \textit{universe of values}.
This could be, for example, a set consisting of all numbers, functions $\bb R\times\bb R\longto\bb R$, tuples of numbers, etc.
...
That is, the initial $\beta$-reducer takes two \kinds{} from $\Sigma$ and returns the following:
\begin{enumerate}
    \item a new \kind,
    \item a function to determine the priority of the new \kind{} given the priorities of the original two \kinds,
    \item and a function to determine the value of the new \kind{} given the values of the original two \kinds.
\end{enumerate}

Define $\oP=\oS\times\values$ to be the set of all possible terms,
that is, 
a term consists of a \kind, a priority, and a value.
...
Since terms now consist of three parts, we write them as $\sigma_p(v)$, where $\sigma$ is the \kind; $p$ is the priority; and $v$ the value.
...
\begin{example}
    ...
    And we define our universe of values to consist of
    \begin{enumerate}
        \item \emph{none}: a value which reflects that the token has no value,
        \item numbers ($\bb R$),
        \item binary operators (functions $\bb R\times\bb R\to\bb R$),
        \item pairs consisting of a number and a binary operator.
    \end{enumerate}

    Our initial $\beta$-reducer is defined as follows:
    \begin{itemize}
        \item $\hb(\NUM,\OP)=(\OP\NUM,\snd,(v,f\mapsto(v,f))$ (here $v$ is a number, $f$ a binary operator, and the result is a pair),
    ...
    \end{itemize}
    In this approach, as opposed to the approach taken in the previous section, we simplify the reduction rules (especially when it comes to the rules regarding operations).
    ...
    In this example allocating initial priorities is not so simple: the priorities depend not on the \kind{} of the token in $\Sigma$, but the token in the stream given to the lexer.
    So we will assume the lexer allocates priorities similar to example \ref{example:partial}.
    ...
    The boxes are removed from around the tokens because they were an aesthetic quirk we used for readability.
    Since each token is now easily distinguishable from other tokens, the boxes are no longer necessary.
    ...
    \begin{gather*}
        ...
        \NUM_0(12)
    \end{gather*}
    We remove parentheses for added readability when it leads to no confusion: for example we write $\OP\NUM_2(2,*)$ instead of $\OP\NUM_2((2,*))$.
    And we don't write the value if it's \emph{none} (e.g. in the case of $\LPAREN_\infty$).
...
\subsubsection{Base Sets}

We begin the description of stateful reduction by describing certain sets which must exist for the algorithm.
...
The empty point-state (with the empty domain) will be denoted by $\varnothing$.
...
The set of indices represent the \emph{closures} of the state: every function call will push a new point-state and index to the state.
This way if a function returns another function, it can look at its current closure and return it as well, which would allow the returned function to reference variables in its parent's scope.
...
\begin{example} \label{example:state}

    Let $\Sigma^0=\set\NUM$, $\Sigma^1=\set\LIST$, so that $\Sigma=\Sigma^0\cup\Sigma^1$; and $\c A^0=\set{\SEMI,\LBRACK,\RBRACK}$, $\c A^1=\set\OP$, so that $\c A=\c A^0\cup\c A^1$.
    Now suppose that the printable terms, $\termp$, consists of variable names: $\mb{x_1},\mb{x_2},\dots$, as well as symbols for numbers $\mbt{1},\mbt{2},\dots$, symbols for binary operations $\mbt+,\mbt*$, and the
    symbols $\mbt;,\mbt[,\mbt]$.
    For readability, we again return to putting boxes around printable terms (but only \textit{printable} terms).

    The set of values $\values$ is defined as follows:
    \begin{enumerate}
        \item \ttt{none} is a value,
        \item every number is a value,
        \item every list of non-\ttt{none} values is a value, i.e. if $v_1,\dots,v_n\in\values$ and $v_i\neq\ttt{none}$ then $(v_1,\dots,v_n)\in\values$.
    \end{enumerate}

    Our initial state, $s_1$ will map numbers $\mbt{n}$ to $\NUM(n)$, binary operations $\mb\circ$ to $\OP(\circ)$, and $\mbt;\mapsto\SEMI,\mbt[\mapsto\LBRACK,\mbt]\mapsto\RBRACK$.
    The program's state will be $\bar s=[(s_1),\;(1)]$.

    Now suppose our program sets $\mb{x_1}$ to the list $(1,2,3)$, then we will want our program's state to be $\bar s=\bar s[\mb{x_1}\mapsto\LIST\NUM((1,2,3))]$.
    And suppose now that our program enters a block, then we will set our program's state to $\bar s=\bar s+\varnothing$.
    Upon exiting the block, we set our program's state to $\bar s=\pop\bar s$.

    Now suppose our program wants to a call a function which has parameters $\mb{x_1},\mb{x_2},\mb{x_3}$ and a closure $s$ with the input $1,2,((3,4),(5,6))$.
    Then our program's state will be
    \[ \bar s = (\bar s +_c s)\bigl[\mb{x_1}\mapsto\NUM(1),\mb{x_2}\mapsto\NUM(2),\mb{x_3}\mapsto\LIST(\LIST\NUM)((3,4),(5,6))\bigr] \]
    We do $+_c$ instead of a normal $+$ because we are creating another scope.
    Now suppose we define a function inside this scope, then we will need to take $\bar s_c$ and add this to the function so it can access variables from this scope.

\end{example}

\subsubsection{The $\beta$-Reducer}

Now we can describe the initial $\beta$-reducer and its derived $\beta$-reducer.

As before, we have an \textit{initial priority function}, which is a function $\pi\colon\termp\longto\bbZ$.
...
Finally, our \textit{initial $\beta$-reducer} is a function
...
\begin{example}
    ...
    Our base sets and initial state will be defined as in example \ref{example:state}.
    ...
    \begin{itemize}
        ...
        \item $\hb(\OP\NUM,\OP\NUM)=(\OP\NUM,\snd,((v,f),(u,g)\mapsto (f(v,u),g)))$
        \item $\hb(\sigma,\SEMI)=(\sigma,\zero,(v,\_\mapsto v))$
    ...
    \mbt[_0\ \mbt1_\infty\ \mbt+_1\ \mbt2_\infty\ \mbt;_0\ \mbt3_\infty\ \mbt*_2\ \mbt4_\infty\ \mbt;_0\ \mbt]_0\cr
        ...
        &\LBRACK_0\ \OP\NUM_1(1,+)\ \NUM_\infty(2)\ \SEMI_0\ \mbt3_\infty\ \mbt*_2\ \mbt4_\infty\ \mbt;_0\ \mbt]_0\cr
        ...
        &\LBRACK\NUM_0((3))\ \OP\NUM_2(3,*)\ \NUM_\infty(4)\ \SEMI_0\ \mbt]_0\cr
...
\begin{enumerate}
    ...
    \item $\Gamma$ is a finite alphabet of non-terminals (we don't require $\Sigma,\Gamma$ to be disjoint),
    ...
    \item $\hb$ is an initial $\beta$-reducer, which is a partial function $\hb\colon(\Gamma\cup\set\square)\times(\Gamma\cup\set{\epsilon,\blacksquare})\partial\Gamma\cup\set{\epsilon,\bullet}$.
\end{enumerate}
...
According to theorem \gobble\ref{thrm:complexity}, all $\beta$-reducible languages can be reduced by their $\beta$-reducer in $O(n)$ time.

For example, $\set{a^nb^n}[n\geq1]$ is in \BR.
We define $B=(\Sigma,\Gamma,\square,\blacksquare,\bullet,s,\hb)$ as follows:
\begin{enumerate}
    \item $\Sigma=\set{a,b}$,
    \item $\Gamma=\set{S_a,S_b,S_{ab},S_{aab}}$,
    \item $\square,\blacksquare,\bullet$ are all distinct elements not in $\Sigma,\Gamma$,
...
\[ \hb\colon\Gamma\times(\Gamma\cup\set{\epsilon,\blacksquare})\partial(\Gamma\cup\set{\epsilon,\bullet})\times(\bbN\times\bbN\to\bbN) \]
and we have an initial priority function $\pi\colon\Sigma\to\bbN$.
Let $F$ be the set of priority functions returned by $\hb$, that is
\[ F = \set{f\colon\bbN\times\bbN\to\bbN}[\exists\sigma\in\Gamma,\tau\in\Gamma\cup\set{\epsilon,\blacksquare}\colon\hb(\sigma,\tau)=(\alpha,f)] \]
and let $N=\mathrm{Im}\pi$ be the set of all initial priorities.
Now let us define $N_0=N$ and $N_{n+1} = N_n\cup\set{f(n,m)}[n\geq m\in N_n, f\in F]$.
...
\begin{enumerate}
    ...
    \item If $\hb(\sigma,\tau)=(\alpha,f)$ then for $n\geq m\in N_\infty$ we define $\hb'(\sigma_n,\tau_m)=\alpha_{f(n,m)}$.
        Note that by definition $f(n,m)\in N_\infty$ so this is well-defined.
\end{enumerate}
...
\subsection{Variants of \BR}

In this section we define variants of $\beta$-reducible languages, which are all special cases (including the initial variant
already defined) of the following definition:

\begin{definition}

    A \emph{nondeterministic $n$-\BR{} schema} (or an $n$-\NBR{} schema), for $n\in\bN$, is a tuple
    $B=(\Sigma,\Gamma,\square,\blacksquare,\bullet,s,\hb)$ (like before), where
    \begin{enumerate}
        \item $\Sigma$ is a finite alphabet of terminals;
        \item $\Gamma$ is a finite alphabet of non-terminals (not necessarily disjoint from $\Sigma$);
        \item $\square,\blacksquare$ are symbols not in $\Sigma,\Gamma$;
        \item $\bullet$ is a symbol (perhaps in $\Sigma$ or $\Gamma$), or the empty word;
        \item $s$ is a state, which is a map $\Sigma\to\Gamma$;
        \item $\hb$ is an initial $\beta$-reducer, which is a function
        $$ \hb\colon(\Gamma\cup\set\square)\times(\Gamma\cup\set{\epsilon,\blacksquare})\to
        \P((\Gamma\cup\set{\epsilon,\bullet})^n\cup\set\perp) $$
    \end{enumerate}
...
\begin{definition}
    ...
    Given a nondeterministic $n$-\NBR{} schema $B$, we define the function
    $\beta'\colon(\Gamma\cup\set\bullet)^*\to\P((\Gamma\cup\set\bullet)^*)$ as follows:
    \begin{enumerate}
        \item $\beta'(\bullet)=\set\bullet$;
        \item $\beta'(\sigma)=\hb(\sigma,\blacksquare)$ if defined, otherwise $\set\sigma$;
        \item if $\xi=\sigma_1\sigma_2\xi'$, then
        \begin{enumerate}
            \item if $\perp\notin\hb(\sigma_1,\sigma_2)$, $\beta'(\xi)=\hb(\sigma_1,\sigma_2)\xi'$
            (where the concatenation of sets is defined word-wise) if defined, and if $\perp$ is in the reduction, then take
            the union of this with $\sigma_1\
            otherwise
            \item $\beta'(\xi)=\hb(\sigma_1,\epsilon)\xi'$ if defined, otherwise
            \item $\beta'(\xi)=\sigma_1\beta'(\sigma_2\xi')$.
        \end{enumerate}
    \end{enumerate}
    Then we define the function $\beta\colon(\Gamma\cup\set\bullet)^*\to\P((\Gamma\cup\set\bullet)^*)$ as follows:
    \begin{enumerate}
        \item $\beta(\epsilon)=\hb(\square,\blacksquare)$ if defined, otherwise $\set\epsilon$;
        \item $\beta(\sigma\xi)=\hb(\square,\sigma)\xi$ if defined, otherwise:
        \item $\beta(\xi)=\beta'(\xi)$.
    \end{enumerate}
    This naturally extends to a function $\beta\colon\P((\Gamma\cup\set\bullet)^*)\to\P((\Gamma\cup\set\bullet)^*)$.
    We then define the \emph{language of $B$} to be
    $$ \L(B) = \set{w\in\Sigma^*}[\bullet\in\beta^k(s(w))\hbox{ for some $k\geq0$}] $$

\end{definition}

The class $n$-\NBR{} is defined to be the class of all languages recognized by a nondeterministic $n$-\BR{} schema.
Then we define \sNBR{} to be the union of all $n$-\NBR{} for $n\geq1$.

\begin{definition}

    An $n$-\NBR{} schema is \emph{deterministic} if 
